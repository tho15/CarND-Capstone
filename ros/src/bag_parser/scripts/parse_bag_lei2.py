...
def parse_rosbag():
  ...
  red_traffic_light_image_folder = output_folder + "red/"
  green_traffic_light_image_folder = output_folder + "green/"
  yellow_traffic_light_image_folder = output_folder + "yellow/"

  if not os.path.exists(red_traffic_light_image_folder):
    os.makedirs(red_traffic_light_image_folder)
  if not os.path.exists(green_traffic_light_image_folder):
    os.makedirs(green_traffic_light_image_folder)
  if not os.path.exists(yellow_traffic_light_image_folder):
    os.makedirs(yellow_traffic_light_image_folder)

  assert(os.path.isdir(red_traffic_light_image_folder))
  assert(os.path.isdir(green_traffic_light_image_folder))
  assert(os.path.isdir(yellow_traffic_light_image_folder))
      ...
      if car_pose_msg_dt <= MSG_TIME_SYNCHRONIZATION_THRESHOLD \
        ...
        if traffic_light_index0 == traffic_light_index1 and \
          ...
          if distance_to_traffic_light0 >= DISTANCE_TO_TRAFFIC_LIGHT_LOWER_BOUND \
            ...
            if traffic_light_state0==TRAFFIC_LIGHT_RED:
              image_path = red_traffic_light_image_folder + image_name
            elif traffic_light_state0==TRAFFIC_LIGHT_GREEN:
              image_path = green_traffic_light_image_folder + image_name
            elif traffic_light_state0==TRAFFIC_LIGHT_YELLOW:
              image_path = yellow_traffic_light_image_folder + image_name
            else:
              assert(0)
